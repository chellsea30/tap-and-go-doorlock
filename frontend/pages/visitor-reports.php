<?php
/**
 * Tap-and-Go Doorlock - Visitor Reports
...
// ============================================================
// GET TOTAL VISITOR LOGS FOR PAGINATION - PERMANENT
// ============================================================
$countQuery = "
...
if (!empty($statusFilter)) {
    $countQuery .= " AND vl.access_status = '$statusFilter'";
}
if (!empty($searchFilter)) {
    $countQuery .= " AND (
        vl.visitor_name LIKE '%$searchFilter%' 
        OR u.full_name LIKE '%$searchFilter%'
        OR vl.purpose_of_visit LIKE '%$searchFilter%'
    )";
}
...
// ============================================================
// GET VISITOR LOGS - PERMANENT (NO DELETE)
// ============================================================
$logs = [];
$query = "
    SELECT 
...
if (!empty($statusFilter)) {
    $query .= " AND vl.access_status = '$statusFilter'";
}
if (!empty($searchFilter)) {
    $query .= " AND (
        vl.visitor_name LIKE '%$searchFilter%' 
...
$result = $conn->query("
    SELECT COUNT(*) as count 
    FROM visitor_logs vl
    WHERE $dateCondition AND vl.access_status = 'granted'
");
if ($result && $row = $result->fetch_assoc()) {
...
$result = $conn->query("
    SELECT COUNT(*) as count 
    FROM visitor_logs vl
    WHERE $dateCondition AND vl.access_status = 'denied'
");
if ($result && $row = $result->fetch_assoc()) {
...
");
if ($result && $row = $result->fetch_assoc()) {
    $stats['unique_visitors'] = (int)$row['count'];
}

$result = $conn->query("
    SELECT COUNT(DISTINCT vl.resident_visited) as count 
    FROM visitor_logs vl
    WHERE $dateCondition AND vl.resident_visited IS NOT NULL
");
if ($result && $row = $result->fetch_assoc()) {
    $stats['residents_visited'] = (int)$row['count'];
}

// ============================================================
// GET CHART DATA - DAILY VISITOR TRENDS
// ============================================================
$chartData = [];
$chartQuery = "
    SELECT 
...
<head>
    ...
    <title>Visitor Reports - Tap-and-Go Doorlock</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .visitor-table .user-cell { display: flex; align-items: center; gap: 10px; }
        .visitor-table .user-avatar {
            width: 32px;
            height: 32px;
            border-radius: 50%;
            display: flex;
        ...
        .visitor-table .user-avatar img {
            width: 100%;
            height: 100%;
            object-fit: cover;
        }
        .visitor-table .user-avatar .no-photo {
            display: flex;
            align-items: center;
            justify-content: center;
            width: 100%;
        ...
        .visitor-table .denied-row:hover td {
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">
                        <i class="fas fa-user-friends me-2" style="color: #1a3a6a;"></i>
                        Visitor Reports
                ...
                <div class="no-delete-banner mb-3">
                    ...
                    <div>
                        ...
                        <span class="text-muted ms-2">Visitor logs are stored permanently for record keeping. Data cannot be deleted to maintain accurate historical records.</span>
                    </div>
                </div>
                ...
                <div class="filter-section">
                    ...
                    <form method="GET" action="" class="row g-2 align-items-end mt-2">
                        ...
                        <div class="col-md-3">
                            ...
                            <input type="text" class="form-control" name="search" placeholder="Visitor Name, Resident, or Purpose" value="<?php echo htmlspecialchars($searchFilter); ?>">
                        </div>
                        <div class="col-md-3">
                ...
                <div class="chart-container">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <h6 style="color: #e0e0e0; margin: 0;">
                            ...
                            Daily Visitor Trends - <?php echo $dateLabel; ?>
                        </h6>
                                    ...
                                    <?php else: ?>
                                        ...
                                        ?>
                                            <tr class="<?php echo $isDenied ? 'denied-row' : ''; ?>">
                                                ...
                                                <td>
                                                    <div class="user-cell">
                                                        <div class="user-avatar resident-avatar">
                                                            <?php if ($hasPhoto): ?>
                                                                <img src="<?php echo $photoUrl; ?>" alt="<?php echo htmlspecialchars($residentName); ?>" 
                                                                     onerror="this.style.display='none'; this.parentElement.innerHTML='<div class=\'no-photo\'>' + '<?php echo $residentInitials; ?>' + '</div>'">
                                                            <?php else: ?>
                                                                <div class="no-photo"><?php echo $residentInitials; ?></div>
                                                            <?php endif; ?>
                                                        </div>
                                                        <div>
                                                            <div><?php echo htmlspecialchars($residentName); ?></div>
                                                            <?php if (!empty($log['resident_room'])): ?>
                                                                <div style="font-size: 10px; color: #808090;">
                                                                    <i class="fas fa-bed me-1"></i>
                                                                    Room <?php echo htmlspecialchars($log['resident_room']); ?>
                                                                </div>
                                                            <?php endif; ?>
                                                        </div>
                                                    </div>
                                        ...
                                        <nav aria-label="Page navigation">
                                            <ul class="pagination justify-content-end mb-0">
                                                <li class="page-item <?php echo ($page <= 1) ? 'disabled' : ''; ?>">
                                                    <a class="page-link" href="?page=1&period=<?php echo $period; ?>&date=<?php echo urlencode($dateFilter); ?><?php echo !empty($statusFilter) ? '&status=' . urlencode($statusFilter) : ''; ?><?php echo !empty($searchFilter) ? '&search=' . urlencode($searchFilter) : ''; ?><?php echo '&per_page=' . $perPage; ?>">
                                                        <i class="fas fa-angle-double-left"></i>
                                                    </a>
                                                </li>
                                                <li class="page-item <?php echo ($page <= 1) ? 'disabled' : ''; ?>">
                                                    <a class="page-link" href="?page=<?php echo $page - 1; ?>&period=<?php echo $period; ?>&date=<?php echo urlencode($dateFilter); ?><?php echo !empty($statusFilter) ? '&status=' . urlencode($statusFilter) : ''; ?><?php echo !empty($searchFilter) ? '&search=' . urlencode($searchFilter) : ''; ?><?php echo '&per_page=' . $perPage; ?>">
                                                ...
                                                ?>
                                                    <li class="page-item <?php echo ($i == $page) ? 'active' : ''; ?>">
                                                        <a class="page-link" href="?page=<?php echo $i; ?>&period=<?php echo $period; ?>&date=<?php echo urlencode($dateFilter); ?><?php echo !empty($statusFilter) ? '&status=' . urlencode($statusFilter) : ''; ?><?php echo !empty($searchFilter) ? '&search=' . urlencode($searchFilter) : ''; ?><?php echo '&per_page=' . $perPage; ?>">
                                                            <?php echo $i; ?>
                                                        </a>
                                                    </li>
                                                <?php endfor; ?>
                ...
                <div class="text-center text-muted small mt-2">
                    ...
                    Total: <?php echo $stats['total']; ?> visitor logs recorded
                    ...
                    <?php echo $stats['residents_visited']; ?> residents visited
                    <span class="mx-1">|</span>
