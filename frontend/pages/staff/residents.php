 ...
 * FIXED: HTTP ERROR 500 - Removed problematic queries
 */
...
// ============================================================
try {
    // Count total residents - simplified
    ...
    // Get residents - simplified query without joins that might fail
    $query = "
        SELECT 
            u.user_id,
            u.full_name,
    ...
    while ($row = $result->fetch_assoc()) {
        // Check if resident has card
        $cardQuery = "SELECT card_uid, status FROM rfid_cards WHERE user_id = ? AND status = 'active' LIMIT 1";
        $cardStmt = $conn->prepare($cardQuery);
        $cardStmt->bind_param("i", $row['user_id']);
        $cardStmt->execute();
        $cardResult = $cardStmt->get_result();
        $cardData = $cardResult->fetch_assoc();
        $cardStmt->close();
        
        $row['card_uid'] = $cardData['card_uid'] ?? null;
        $row['card_status'] = $cardData['status'] ?? null;
        
        // Get resident profile
        $profileQuery = "SELECT course, year_level, gender, age FROM resident_profiles WHERE user_id = ?";
        ...
        $row['course'] = $profileData['course'] ?? 'N/A';
        $row['year_level'] = $profileData['year_level'] ?? 'N/A';
        $row['gender'] = $profileData['gender'] ?? 'N/A';
        $row['age'] = $profileData['age'] ?? 'N/A';
        
        // Get admission status
        $admissionQuery = "SELECT status as admission_status, room_assignment FROM admission_records WHERE user_id = ? ORDER BY created_at DESC LIMIT 1";
        $admissionStmt = $conn->prepare($admissionQuery);
        $admissionStmt->bind_param("i", $row['user_id']);
        $admissionStmt->execute();
        $admissionResult = $admissionStmt->get_result();
        $admissionData = $admissionResult->fetch_assoc();
        $admissionStmt->close();
        
        $row['admission_status'] = $admissionData['admission_status'] ?? 'pending';
        $row['room_assignment'] = $admissionData['room_assignment'] ?? null;
